...
function SeverityBadge({ severite }: { severite: string }) {
  ...
  const cfg =
    s === 'absolue' || s === 'contre_indication' || s === 'contre-indication absolue' || s === 'majeure'
      ? { cls: 'bg-red-100 text-red-800 border-red-200', label: s === 'contre_indication' ? 'CI' : s }
      : s === 'relative' || s === 'moderee' || s === 'modérée'
      ? { cls: 'bg-orange-100 text-orange-800 border-orange-200', label: s === 'moderee' ? 'modérée' : s }
    ...
    const load = async () => {
      ...
      const [cRes, iRes] = await Promise.all([
        supabase.from('contraindications')
          ...
          .ilike('dci_pattern', `%${dciLower}%`).limit(50),
        supabase.from('drug_interactions')
          ...
          .in('severite', ['contre_indication', 'majeure']).limit(50),
      ]);
          ...
          <>
                ...
                : <div className="space-y-2">
                    {contras.map((c, i) => (
                      <div key={i} className="bg-red-50 dark:bg-red-500/[0.07] rounded-xl p-3 border border-red-100 dark:border-red-500/20">
                        ...
                        {c.mecanisme_fr && <p className="text-[10px] text-slate-400 mt-1 italic">⚙ {c.mecanisme_fr}</p>}
            ...
            <section>
              <h3 className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide mb-2 flex items-center gap-1.5">
                ...
                Interactions graves ({interactions.length})
              </h3>
              {interactions.length === 0
                ? <p className="text-xs text-slate-400 italic">Aucune interaction grave enregistrée.</p>
                : <div className="space-y-2">
                    {interactions.map((it, i) => {
                      const partner = med.dci
                        ? (it.dci_1_pattern.toLowerCase().includes(med.dci.toLowerCase()) ? it.dci_2_pattern : it.dci_1_pattern)
                        : it.dci_2_pattern;
                      return (
  ...
  const fetchPaths = useCallback(async (q: string, page: number, cat: string) => {
    ...
    try {
      ...
      let query = supabase
        ...
        .order('nom_fr').range(from, to);

      if (q.trim().length >= 1) {
        query = query.or(`nom_fr.ilike.%${q}%,nom_en.ilike.%${q}%,icd10_code.ilike.%${q}%`);
      }
      if (cat) {
                  ...
                  {paths.map(path => (
                    <button key={path.id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {path.nom_en && <p className="text-xs text-slate-500 dark:text-slate-400 italic truncate">{path.nom_en}</p>}
